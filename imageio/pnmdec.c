...
static size_t ReadPAMFields( PNMInfo* const info, size_t off )
{
	...
	while( 1 )
	{
		...
		else
		{
			...
			int i;
			if( out_size > 20 ) sprintf_s( out + 20 - strlen( kEllipsis ), MAX_LINE_SIZE, kEllipsis );
...
static size_t ReadHeader( PNMInfo* const info )
{
	...
	off = ReadLine( info->data, off, info->data_size, out, &out_size );
	if( off == 0 || sscanf_s( out, "P%d", &info->type ) != 1 ) return 0;
	if( info->type == 7 )
	{
	...
	{
		off = ReadLine( info->data, off, info->data_size, out, &out_size );
		if( off == 0 || sscanf_s( out, "%d %d", &info->width, &info->height ) != 2 )
		{
			return 0;
		}
		off = ReadLine( info->data, off, info->data_size, out, &out_size );
		if( off == 0 || sscanf_s( out, "%d", &info->max_value ) != 1 ) return 0;
