					...
					WebPPicture* const pic )
{
	...
	const size_t expected_data_size = y_plane_size + (size_t)2 * uv_plane_size;
						...
						int keep_alpha, Metadata* const metadata )
{
	...
	ok = xImgIoUtilReadFile( filename, &data, &data_size );
	if( !ok ) goto End;
...
static void AllocExtraInfo( WebPPicture* const pic )
{
	...
	pic->extra_info =
		(uint8_t*)WebPMalloc( (size_t)mb_w * mb_h * sizeof( *pic->extra_info ) );
}
...
static int DumpPicture( const WebPPicture* const picture, const char* PGM_name )
{
	...
	FILE* f = NULL;
	_wfopen_s( &f, (wchar_t*)PGM_name, L"wb" );
	if( f == NULL ) return 0;
	fprintf( f, "P5\n%d %d\n255\n", stride, height );
...
//	INIT_WARGV( argc, argv );
	...
	if( out_file != NULL )
	{
		...
		if( use_stdout )
		{
			out = ImgIoUtilSetBinaryMode( stdout );
		}
		else
		{
