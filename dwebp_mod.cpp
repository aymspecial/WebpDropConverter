					   ...
					   WebPOutputFileFormat format, const char* const out_file, int jpgQuality = 80 )
{
	...
	ok = WebPSaveImage( buffer, format, out_file, jpgQuality );
	...
	if( format >= RGB && format <= rgbA_4444 )
	{
		const int bpp = ( format == RGB || format == BGR ) ? 3
			: ( format == RGBA_4444 || format == rgbA_4444 ||
				format == RGB_565 ) ? 2
			: 4;
		size_t stride = (size_t)bpp * w + 7;   // <- just for exercising
...
int dwebp_main( int argc, char* argv[] )
{
	...
	for( c = 1; c < argc; ++c )
	{
		...
		else if( !strcmp( argv[ c ], "-crop" ) && c < argc - 4 )
		{
			...
			config.options.crop_height = ExUtilGetInt( argv[ ++c ], 0, &parse_error );
				 ...
				 c < argc - 2 )
		{  // '-scale' is left for compatibility
	...
	if( out_file != NULL )
	{
		...
		ok = SaveOutput( output_buffer, format, out_file, jpgQuality );
	}
	else
	{
		if( !quiet )
